package cool.scx.http.media.json;
...
/// JSON Reader 默认先将请求体读取为字符串，然后解析为 JsonNode。
/// 原因说明：
/// - 在大多数 Web 场景中，请求体大小通常 < 200KB；
/// - 经测试，在这种数据量下，先读取为字符串的方式通常性能更好（避免流式初始化、便于字符集处理）；
/// - 同时具有更强的容错性和调试便利性（可打印原始字符串）；
/// 若后续需要处理大 JSON（如 >1MB）或高并发大吞吐场景，
/// 可考虑替换为基于 InputStream 的流式解析器。
///
/// @author scx567888
/// @version 0.0.1
public class JsonReader implements MediaReader<JsonNode> {
