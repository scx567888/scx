 ...
 */
@Deprecated
public class OldMySQLDao<Entity> implements BaseDao<Entity> {

    /**
     * 实体类对应的 table 结构
     */
    protected final TableInfo<? extends OldMySQLDaoColumnInfo> tableInfo;
     ...
     * @param sqlRunner   a
     */
    public OldMySQLDao(TableInfo<? extends OldMySQLDaoColumnInfo> tableInfo, Class<Entity> entityClass, SQLRunner sqlRunner) {
     ...
     *  }</pre>
     * <br>
     * 注意 !!! 若同时使用 limit 和 in/not in 请使用 {@link OldMySQLDao#buildSelectSQLWithAlias(Query, SelectFilter)}
     *
    ...
    public final SQL buildSelectSQLWithAlias(Query query, SelectFilter selectFilter) {
        ...
        if (query.pagination().rowCount() == null) {
            ...
            var sql0 = Select(selectColumns)
                    ...
                    .Limit(query.pagination().offset(), query.pagination().rowCount())
                    .GetSQL();
            var sql = Select(Arrays.stream(selectColumnInfos).map(OldMySQLDaoColumnInfo::javaFieldName).toArray(String[]::new)).From("(" + sql0 + ")").GetSQL();
    ...
    @Override
    public final TableInfo<? extends OldMySQLDaoColumnInfo> _tableInfo() {
