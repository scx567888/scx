...
public final class FromPathMethodParameterHandler implements ScxMappingMethodParameterHandler {
     ...
     */
    public static Object getValueFromPath(String name, boolean merge, boolean required, JavaType javaType, ScxMappingRequestInfo requestInfo) throws RequiredParamEmptyException, ParamConvertException {
        var tempValue = getFromMap(name, requestInfo.routingContext().pathParams(), merge);
        if (tempValue == null) {
            if (required) {
    ...
    public Object handle(Parameter parameter, ScxMappingRequestInfo requestInfo) throws Exception {
        ...
        return getValueFromPath(name, merge, required, javaType, requestInfo);
    }
