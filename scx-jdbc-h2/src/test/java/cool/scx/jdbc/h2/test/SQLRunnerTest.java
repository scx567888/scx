package cool.scx.jdbc.h2.test;

import cool.scx.jdbc.JDBCContext;
import cool.scx.jdbc.h2.test.bean.Student;
import cool.scx.jdbc.h2.test.bean.StudentRecord;
...
public class SQLRunnerTest {
    ...
    static {
        ...
        try {
            AppRoot = ClassUtils.getAppRoot(ClassUtils.getCodeSource(SQLRunnerTest.class));
            TempSQLite = AppRoot.resolve("temp").resolve("temp");
            Files.createDirectories(TempSQLite.getParent());
        } catch (URISyntaxException | IOException e) {
            throw new RuntimeException(e);
        }
        dataSource = getH2Source();
...
//        test6();
    ...
    public static void beforeTest() {
        try {
            sqlRunner.execute(sql("drop table if exists " + tableName + ";" + " create table " + tableName + "(`name` varchar(32) unique ,`age` integer,`sex` boolean )"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//    @Test
    ...
    private static DataSource getH2Source() {
        var h2DataSource = new JdbcDataSource();
        h2DataSource.setUrl("jdbc:h2:" + TempSQLite);
        return Spy.wrap(h2DataSource);
    }
