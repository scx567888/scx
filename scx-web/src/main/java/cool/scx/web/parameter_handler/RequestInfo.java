...
public final class RequestInfo {
    ...
    public RequestInfo(RoutingContext ctx, boolean cachedMultiPart) {
        ...
        this.pathParams = ScxObject.convertValue(ctx.pathParams().toMultiValueMap(), ObjectNode.class);
        this.query = ScxObject.convertValue(ctx.request().query().toMultiValueMap(), ObjectNode.class);
        this.bodyInit = false;
    }
    ...
    private void initBody(RoutingContext ctx, ScxMediaType contentType) {
        ...
        if (APPLICATION_X_WWW_FORM_URLENCODED.equalsIgnoreParams(contentType)) {
            ...
            this.body = ScxObject.convertValue(formParams.toMultiValueMap(), Node.class);
            return;
        }
        if (MULTIPART_FORM_DATA.equalsIgnoreParams(contentType)) {
            ...
            this.body = ScxObject.convertValue(m.toMultiValueMap(), Node.class);
            this.uploadFiles = f;
            return;
        }
