...
import cool.scx.orm.jdbc.result_handler.ResultHandler;
import cool.scx.orm.jdbc.sql.SQL;
import cool.scx.orm.query.GroupBy;
import cool.scx.orm.query.GroupByBody;
import cool.scx.orm.query.OrderBy;
import cool.scx.orm.query.OrderByBody;
...
import static cool.scx.orm.jdbc.SQLBuilder.*;
import static cool.scx.orm.jdbc.dialect.DialectSelector.findDialect;
import static cool.scx.orm.jdbc.result_handler.ResultHandler.ofBeanList;
import static cool.scx.orm.jdbc.result_handler.ResultHandler.ofSingleValue;
...
public class JDBCDao<Entity> implements BaseDao<Entity, Long> {
    ...
    public final SQL buildSelectSQLWithAlias(Query query, ColumnFilter selectFilter) {
        ...
        if (query.limit().rowCount() == null) {
            ...
            var selectColumnInfos = selectFilter.filter(tableInfo);
            var selectColumns = Arrays.stream(selectColumnInfos).map(cool.scx.orm.ColumnMapping::name).toArray(String[]::new);
            var whereClauseAndWhereParams = whereParser.parseWhere(query.where());
            var groupByColumns = getGroupByColumns(query.groupBy(), tableInfo);
            var orderByClauses = getOrderByClauses(query.orderBy(), tableInfo);
