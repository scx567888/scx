...
import static cool.scx.orm.jdbc.ColumnNameParser.parseColumnName;
import static cool.scx.orm.jdbc.ColumnNameParser.splitIntoColumnNameAndFieldPath;
import static cool.scx.util.ObjectUtils.Option;
...
interface WhereTypeHandler {
    ...
    WhereTypeHandler IN_HANDLER = (tableInfo, name, whereType, value1, value2, info) -> {
        ...
        if (value1 instanceof SQL a) {
            ...
            //移除空值并去重
            whereParams = Arrays.stream(ObjectUtils.toObjectArray(value1)).filter(Objects::nonNull).distinct().toArray();
            //长度为空是抛异常
    ...
    WhereTypeHandler JSON_CONTAINS_HANDLER = (tableInfo, name, whereType, value1, value2, info) -> {
        ...
        if (value1 instanceof SQL a) {
            ...
            if (info.useOriginalValue()) {
                ...
                try {
                    whereParams = new Object[]{ObjectUtils.toJson(value1, Option.IGNORE_JSON_IGNORE, Option.IGNORE_NULL_VALUE)};
    ...
    WhereClauseAndWhereParams getWhereParamsAndWhereClause(Table<?> tableInfo, String name, WhereType whereType, Object value1, Object value2, WhereOption.Info info);
