...
///
///   - 1. 系统异常 (如 accept 异常) : 这类错误无法被用户干预, 只能记录日志并关闭 ServerSocket,
///     不需要额外回调来 "处理", 而且也没有什么可处理的.
///   - 2. 用户处理器异常 (如用户在 onConnect 回调中抛出异常) : ScxTCPServer 设计允许用户在 onConnect 回调中直接抛异常,
///     这是为了防止用户遇到受检异常时不得不进行一次 "肮脏的 RuntimeException 包装" 而预留的方式, 当异常抛出时,
///     系统会使用日志记录异常, 并自动关闭对应 socket 以清理资源.
///
