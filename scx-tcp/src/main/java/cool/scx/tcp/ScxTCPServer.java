...
///     由于没有可操作的上下文, 用户既不需要也无法干预, 
///     只需内部记录日志并关闭 ServerSocket 即可, 无需额外暴露 onError 回调.
///
/// 2. **用户处理器异常 (如用户在 onConnect 回调中抛出异常) **
///    用户除了当前 Socket 外, 通常没有更多上下文信息, 即使提供 onError 回调, 能做的也非常有限,
///    只需内部记录日志并关闭 对应 Socket 即可.
///
///    之所以没有 onError 却又允许用户在 onConnect 回调中直接抛出异常, 
///    主要是为了保留最原始的异常信息, 避免用户被迫将受检异常包装成 RuntimeException, 从而减少不必要的封装层级.
///
