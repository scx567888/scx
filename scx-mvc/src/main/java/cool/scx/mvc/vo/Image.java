...
public abstract class Image implements BaseVo {
    ...
    public final void accept(RoutingContext context) throws BadRequestException {
        //设置缓存 减少服务器压力
        context.response()
                .putHeader(HttpHeader.CACHE_CONTROL.toString(), "public,immutable,max-age=2628000")
                .putHeader(HttpHeader.ACCEPT_RANGES.toString(), "bytes");
    ...
    private static final class SystemIconImage extends Image {
        ...
        public void imageHandler(RoutingContext context) {
            context.response().putHeader(HttpHeader.CONTENT_TYPE.toString(), MediaType.IMAGE_PNG.toString()).end(buffer);
        }
    ...
    private static final class CroppedImage extends Image {
        ...
        @Override
        public void imageHandler(RoutingContext context) {
            context.response().putHeader(HttpHeader.CONTENT_TYPE.toString(), contentType).end(buffer);
        }
