...
/// @author scx567888
/// @version 0.0.1
public class UpdateFieldPolicyDeserializer {

    public static final UpdateFieldPolicyDeserializer UPDATE_FIELD_POLICY_DESERIALIZER = new UpdateFieldPolicyDeserializer();

    public UpdateFieldPolicy fromJson(String json) throws JsonProcessingException {
        var v = ObjectUtils.jsonMapper().readTree(json);
        return deserialize(v);
    }

    public UpdateFieldPolicy deserialize(JsonNode v) {
        if (v.isObject()) {
            var type = v.get("@type").asText();
            if (type.equals("UpdateFieldPolicy")) {
    ...
    private UpdateFieldPolicy deserializeFieldPolicy(JsonNode objectNode) {
        var filterMode = EXCLUDED;

        if (objectNode == null) {
            return new UpdateFieldPolicyImpl(filterMode);
        }
        ...
        var fieldPolicy = new UpdateFieldPolicyImpl(filterMode);
