...
class JsonHelper {
    ...
    final static ObjectMapper OBJECT_MAPPER = ObjectUtils.jsonMapper(ObjectUtils.Option.IGNORE_JSON_IGNORE);
    ...
    public static ObjectNode filterObjectNode(ObjectNode objectNode, ColumnFilter filter) {
    ...
    public static DbDoc filterDbDoc(DbDoc dbDoc, ColumnFilter filter) {
        return switch (filter.filterMode()) {
            case EXCLUDED -> {
                for (String s : filter.fieldNames()) {
                    dbDoc.remove(s);
